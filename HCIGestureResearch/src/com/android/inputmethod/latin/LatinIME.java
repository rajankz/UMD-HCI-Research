 ...
 */
public class LatinIME extends InputMethodService implements KeyboardActionListener, OnGesturePerformedListener,
    ...
    private GestureLibrary mLibrary1, mLibrary2,mLibrary3,mLibrary4;
    ...
    public void commitTyped(final InputConnection ic, final int separatorCode) {
        ...
        if (typedWord.length() > 0) {
            if (ic != null) {
                ...
                HCILogger.getInstance().debug(Calendar.getInstance().getTimeInMillis()+"\t"+"TypedText="+typedWord);
                if (ProductionFlag.IS_EXPERIMENTAL) {
    ...
    static private void sendUpDownEnterOrBackspace(final int code, final InputConnection ic) {
        ...
        ic.sendKeyEvent(new KeyEvent(SystemClock.uptimeMillis(), eventTime,
                ...
                KeyEvent.FLAG_SOFT_KEYBOARD | KeyEvent.FLAG_KEEP_TOUCH_MODE));
        HCILogger.getInstance().logKey(code, Calendar.getInstance().getTimeInMillis());
    }
    ...
    public void onGesturePerformed(LatinKeyboardView overlay, Gesture gesture) {
        ...
        switch(gesture.getStrokesCount()){
            ...
            case 3: predictions = mLibrary3.recognize(gesture);break;
            case 4: predictions = mLibrary4.recognize(gesture);break;
            default: predictions = mLibrary.recognize(gesture);
        }
    ...
    private void addPredictedChar(String str){
        ...
        getCurrentInputConnection().commitText(typedWord+str,1);
        mWordComposer.reset();
        updateSuggestions();
    }
